...
- For interoperability, paired Markdown syntax must be complete within each side of a substitution (or enclose the whole substitution from outside). An opener or closer inside one arm cannot pair with the other arm or later source; read-only projections protect any unsafe join and show it literally.
