...
- Pairing markers in a later pass is retroactively unsound: the spike's atom design parses
  markers as inert inline atoms, and emphasis then pairs straight across an annotation closer
  (demonstrated executable, `spikes/parser-hosts/lezer.test.js` atom-design test); a later
  pass that joined those markers would turn the already-made emphasis decision into a
  containment violation (analysis: research 0006 §8).
...
## 5. Ecosystem semantic facts (oracles, not engines)

- MMD-6's accept/reject CuTest matrix is the projection oracle — additions/deletions/
  substitutions/highlights/comments under accept and reject, recursive nesting, unclosed-marker
  passthrough (research 0005; one verifier compiled MMD-6). Where MarkText diverges (stray `~>`
  stays literal here; MMD-6 erases it), the divergence is recorded in the language spec's
  ledger, not here.
...
- Even Word/OOXML — production track changes at maximum hardening — models a cross-paragraph
  ...
  normative semantics is block merging. That boundary-token rule is the accept/reject oracle
  for paragraph-break deletion; the fragment representation is what R-1 rejects (research
  0008).
...
1. micromark's ~2k-test conformance corpus (CommonMark 0.31.2 + GFM), plus its first-party
   math/front-matter extension suites → differential conformance oracle.
...
5. MMD-6's CuTest accept/reject matrix → projection conformance rows.
6. Peritext's gap-anchor semantics (span ends attach to gaps before/after stable atom
   ...
   (research 0008).
7. OOXML's `w:del` paragraph-mark semantics → accept/reject oracle for paragraph-break
   deletion (research 0008).
