...
The plain-paragraph index shares original safe-point facts and path-copies a logarithmic prefix-sum overlay, so those edits do not rebuild the retained-fact suffix. Canonical source is held in an immutable balanced piece rope; eligible edits path-copy it and read only the affected region, while full-string access is a lazy, cached compatibility barrier. Shadow recognition uses the non-materializing source length. This proves the semantic-delta, retained-index, canonical-source, and balanced visible-CriticMarkup seams without putting them on the production input path. The visible-form tracer currently performs two bounded regional parses, and its one-root certificate is built during the already-full open; a very large single safe region can therefore still be expensive. Comment Display, nested or multiple annotations, definitions, structural or marker-affecting edits, generated protective coordinates, option changes, excessively fragmented sources, and full projections still take an explicit whole-document barrier. Production Core mode remains blocked on piece-count maintenance for long barrier-free sessions, a persistent multi-region and nested annotation inventory, a Comment Display delta, replacement of the renderer's snapshot/diff path, and the editor reconciliation and save barriers.
