...
Resource admission carries a conservative global logical-node upper bound and adds each accepted candidate's local accounting without subtracting the replaced contribution. This is safe but can force a false whole-document fallback during a long session. A measured pre-existing debt is more urgent: the current 2,000,000-node desktop policy admits a roughly 2.74 GiB full revision, and a fallback edit can exhaust a 4 GiB Node heap while old and candidate full products coexist; the pushed pre-inventory baseline reproduces the same failure. Before Core-mode production use, calibrate a recoverable policy or make full-product fallback lazy/bounded, then persist the production-limit atomicity case.
