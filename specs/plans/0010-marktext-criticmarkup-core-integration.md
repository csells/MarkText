...
The plain-paragraph index shares original safe-point facts and path-copies a logarithmic prefix-sum overlay, so those edits do not rebuild the retained-fact suffix. Canonical source is held in an immutable balanced piece rope; eligible edits path-copy it and read only the affected region, while full-string access is a lazy, cached compatibility barrier. Shadow recognition uses the non-materializing source length. This proves the semantic-delta, retained-index, canonical-source, balanced visible-CriticMarkup, and Comment Display seams without putting them on the production input path. The visible-form and Comment paths currently parse both the old and new bounded region; a very large single safe region can therefore still be expensive. The Comment fast path currently requires one edit, one top-level Comment target, stable nested CriticMarkup and accounting topology, and a non-edge safe bracket. Empty-Comment authoring, multiple targets or top-level annotations, marker or topology changes, generated main-Markup coordinates, option changes, excessively fragmented sources, and full projections take an explicit whole-document barrier. Production Core mode remains blocked on piece-count maintenance for long barrier-free sessions, a persistent multi-region annotation inventory, a portable full-Comment resynchronization contract, replacement of the renderer's snapshot/diff path, and the editor reconciliation and save barriers.
