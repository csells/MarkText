...
Implementation checkpoint (2026-08-11): the core can now publish a structured-cloneable Markup region replacement for a deliberately narrow, parser-proved plain-paragraph edit. The change includes explicit old and new source, syntax, and event ranges; repeated eligible edits remain regional; older revisions remain projectable; and every ineligible requested change returns a typed whole-document fallback. An immutable regional index shares the original safe-point facts and path-copies only a logarithmic prefix-sum overlay, so eligible edits do not rebuild the untouched retained-fact suffix. Canonical source is held in an immutable balanced piece rope; eligible edits path-copy it and read only the affected region, while full-string access is a lazy, cached compatibility barrier. Shadow recognition uses the non-materializing source length. This proves the semantic-delta, retained-index, and canonical-source seams without putting them on the production input path. CriticMarkup, definitions, structural edits, option changes, excessively fragmented sources, and full projections still take an explicit whole-document barrier. Production Core mode remains blocked on piece-count maintenance for long barrier-free sessions, balanced CriticMarkup-aware regions, replacement of the renderer's snapshot/diff path, and the editor reconciliation and save barriers.
