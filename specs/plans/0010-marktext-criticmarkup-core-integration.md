...
Implementation checkpoint (2026-08-11): the core can now publish a structured-cloneable Markup region replacement for a deliberately narrow, parser-proved plain-paragraph edit. The change includes explicit old and new source, syntax, and event ranges; repeated eligible edits remain regional; older revisions remain projectable; and every ineligible requested change returns a typed whole-document fallback. An immutable regional index shares the original safe-point facts and path-copies only a logarithmic prefix-sum overlay, so eligible edits no longer rebuild the untouched retained-fact suffix. This proves the semantic-delta and retained-index seams without putting them on the production input path. Canonical source is still reconstructed as a whole JavaScript string on each edit, and CriticMarkup, definitions, structural edits, and option changes still take an explicit whole-document fallback. Production Core mode therefore remains blocked on incremental canonical-source storage, balanced CriticMarkup-aware regions, and the editor reconciliation adapter.
