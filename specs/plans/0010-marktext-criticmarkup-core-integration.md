...
This is still a preparatory slice, not general incremental authority. Inventory admission currently requires one edit in one fixed safe region, more than one top-level CM root, stable marker/annotation/main-Markup/Comment-Display topology, no diagnostics, canonical main-Markup coordinates, and unchanged Markdown options. Multiple requested Comments in that edited inventory leaf are normalized and projected locally, including nested targets; targets spanning inventory leaves still take an explicit whole-document barrier. A retained dependency-prefix fingerprint admits unrelated plain edits strictly after unchanged global reference/definition facts, while edits touching or preceding those facts and candidates that introduce new facts retain the typed `definition-or-reference-facts` barrier. Gaps, split/join edits, multiple edited regions, and structural changes remain explicit whole-document barriers. Changed requested Comments retain a local projection; an unchanged Comment requested only as a shifted target is intentionally undelivered, and legacy `projectComment` access crosses the lazy full compatibility barrier. Legacy one-root visible-form and Comment admission still parse both bounded old and new regions. A very large single safe region can therefore still be expensive.
...
Production Core mode remains blocked on a portable document-scope fallback resynchronization payload, broader dependency-directed and cross-leaf regional invalidation, ratified latency evidence, and a current-build installed authority run. Ordinary CodeMirror and Muya edits no longer use renderer snapshot/diff authority, and focused reconciliation, pending-save, composition, generation, and recovery tests cover the manager-owned barriers; those seams still require final release-candidate installed evidence.
