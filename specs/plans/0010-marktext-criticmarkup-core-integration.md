...
Implementation checkpoint (2026-08-11): the core can now publish a structured-cloneable Markup region replacement for a deliberately narrow, parser-proved plain-paragraph edit. The change includes explicit old and new source, syntax, and event ranges; repeated eligible edits remain regional; older revisions remain projectable; and every ineligible requested change returns a typed whole-document fallback. This proves the semantic-delta seam without putting it on the production input path. Retained parse facts are still traversed and copied in proportion to document structure, so this checkpoint does not satisfy the Phase 1A steady-state latency gate. Production Core mode remains blocked on a persistent or paged retained-fact representation, balanced CriticMarkup-aware regions, and the editor reconciliation adapter.
