...
- captured browser input to the exact speculative DOM checkpoint and one external Chromium compositor-surface capture, immediately followed by retained-checkpoint validation; this is an upper bound and not a physical-display, vsync, or next-frame measurement;

- input to authoritative engine acknowledgement;

- renderer/engine transport;

- DOM patch and authenticated external compositor-surface capture;
...
On reference hardware, ordinary input must reach the exact speculative DOM checkpoint at p95 without waiting for persistence or presentation enhancement. The hidden compositor-capture target remains explicitly unratified until an owner freezes a positive target from comparable upstream and Core calibration runs. Engine acknowledgement and reconciliation must normally complete without visible correction or input queue growth. The first viewport must become editable before the remainder of a large document is fully enhanced.
...
This is still a preparatory slice, not general incremental authority. Inventory admission currently requires one edit in one fixed safe region, more than one top-level CM root, at most one requested Comment target, stable marker/annotation/main-Markup/Comment-Display topology, no diagnostics or global reference-definition facts, canonical main-Markup coordinates, unchanged Markdown options, and a source rope that does not require rebasing. Gaps, split/join edits, multiple targets or edited regions, global dependencies, and structural changes take an explicit whole-document barrier. Changed requested Comments retain a local projection; an unchanged Comment requested only as a shifted target is intentionally undelivered, and legacy `projectComment` access crosses the lazy full compatibility barrier. Legacy one-root visible-form and Comment admission still parse both bounded old and new regions. A very large single safe region can therefore still be expensive.

Resource admission carries a conservative global logical-node upper bound and adds each accepted candidate's local accounting without subtracting the replaced contribution. This is safe but can force a false whole-document fallback during a long session.
...
The separate policy-calibration debt remains: the current 2,000,000-node desktop policy can admit a roughly 2.74 GiB single full revision before any history is involved. Before Core-mode production use, calibrate a recoverable limit and persist the production-limit atomicity case without requiring a multi-gigabyte CI fixture.

Production Core mode remains blocked on piece-count maintenance for long barrier-free sessions, a portable full-Comment resynchronization contract, dependency-directed and multi-target regional invalidation, replacement of the renderer's snapshot/diff path, and the editor reconciliation and save barriers.
