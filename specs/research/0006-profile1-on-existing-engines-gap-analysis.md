...
1. **The TypeScript ecosystem was never evaluated as a replacement engine.** Research 0004 was
   ...
   incremental *techniques*. The two strongest adoption candidates for a TS renderer are TS
...
2. **Neither TS candidate is excluded by plan 0009's architecture law.** The law forbids a
   *separate* CM scanner/sidecar and post-hoc reconstruction. A micromark syntax extension runs
   inside micromark's single tokenizer pass — that *is* intrinsic parsing. @lezer/markdown is one
...
3. **The from-scratch engine's remaining work is its riskiest work.** Of document-core's ~15.3k
   ...
   sunk cost is not an argument; the *remaining risk profile* is the argument.
...
- **`micromark`** — wooorm's reference CommonMark tokenizer. **100% CommonMark, 100% GFM**, math
  / frontmatter / directives / MDX all built on its *public* `SyntaxExtension` API (unlike its
...
| P2 losslessness / round-trip | ✅ trivially — tree annotates the source you keep (the CodeMirror model *is* source-authoritative) | ✅ events cover every byte (consume events, not lossy mdast) | 🔧 positions exact; no round-trip guarantee | ✅ by design |
...
and — the bigger item — the *unfinished* work: the CommonMark conformance long tail (the least
...
of Layer D's CM semantics. No candidate provides any of it. The build-vs-adopt question is *only*
...
1. **R3 (multi-block CM) is fork-risk on every host.** It is also the hardest part of the custom
   engine — the lane/fork/safe-point machinery exists *because* of it. Honest statement: R3 is
...
2. **Lezer's link-reference deviation vs the §2 pin.** Lezer deliberately does not validate
   ...
   spec defect for our purposes — and it is the *same* non-locality that makes incremental
...
- **The from-scratch engine is not irrational** — it is the only path that satisfies the spec
  ...
  anything else on earth. But it carries the most *undifferentiated* remaining work.
