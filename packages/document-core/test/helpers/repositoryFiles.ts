...
function globToRegExp(glob: string): RegExp {
  ...
  for (let index = 0; index < glob.length; index += 1) {
    ...
    if (character === '*') {
      ...
      if (end === -1) { throw new Error(`Unclosed alternate in glob ${glob}`) }
...
function matches(compiled: CompiledGlob, relativePath: string): boolean {
  if (compiled.pathwise) { return compiled.expression.test(relativePath) }
...
export function listRepositoryFiles(
  ...
  const walk = (relativeDirectory: string): void => {
    ...
    for (const entry of readdirSync(absolute, { withFileTypes: true })) {
      ...
      if (negatives.some(negative => matches(negative, relativePath))) { continue }
