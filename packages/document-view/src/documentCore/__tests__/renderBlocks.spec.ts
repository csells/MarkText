 ...
 * block tree into real DOM, making this package a view over the engine
...
async function render(source: string): Promise<HTMLElement> {
    ...
    const runs = renderMarkupPlan(snapshot.livePlan);
...
describe('document-core block rendering', () => {
    ...
    it('mounts a boundary-protected unresolved reference as literal text', async () => {
        const source = 'p{--\n\n--}[r]: /{++y++}';
        const session = await createDocumentSession({
            source: createSourceSnapshot(source),
            parseConfiguration: PARSE_CONFIGURATION,
        ...
        renderDocumentCoreBlocks(
            host,
            groupRenderBlocks(
                revised.displayDocument,
                renderMarkupPlan(revised.displayPlan),
            ),
        );

        expect(host.textContent).toBe('p[r]: /{++y++}');
