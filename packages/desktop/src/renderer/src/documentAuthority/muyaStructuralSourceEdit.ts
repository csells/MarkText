...
const annotatedContainerPrefix = (
  ...
  const binding = bindings.find(
    (item) => item.path.length === 2 && item.path[0] === index && item.path[1] === 'text'
  )
  if (
  ...
  if (
    !deepEqual(previous, { name: 'paragraph', text: binding.text }) ||
    next === null ||
    typeof next !== 'object'
  ) { return undefined }
  ...
  const listed =
    (container.name === 'bullet-list' || container.name === 'order-list') &&
    deepEqual(container.children[0], { name: 'list-item', children: [previous] })
  const tasked =
    container.name === 'task-list' &&
    deepEqual(container.children[0], {
      name: 'task-list-item',
      meta: { checked: false },
      children: [previous]
    })
  if (!listed && !tasked) return undefined
  try {
    ...
    const wrapped = {
      ...container,
      children: [{ ...(container.children[0] as object), children: [leaf] }]
    }
...
export function sourceEditForMuyaStructuralChange(
  ...
  if (oldEnd === first) {
    ...
    try {
      ...
      const generated = serializeNativeState(doc.slice(first, newEnd), {
        maximumUnits,
        sourceLineEndings: end.lineEnding
      })
      if (generated === undefined) return undefined
  ...
  if (oldEnd === first + 1 && newEnd === first + 1) {
    const emptyLiteral = bindings.find(
      (binding) =>
        binding.path[0] === first &&
        binding.emptyLiteralLineEnding !== undefined &&
        binding.text === '' &&
    ...
    const prefix = annotatedContainerPrefix(
      bindings,
      first,
      prevDoc[first],
      doc[first],
    ...
    const leaf = bindings.find(
      (binding) => binding.path[0] === first && binding.containerPrefix !== undefined
    )
    if (
      leaf?.annotationContext &&
      leaf.containerPrefix !== undefined &&
      atPath(prevDoc, leaf.path) === leaf.text &&
      deepEqual(doc[first], { name: 'paragraph', text: leaf.text })
    ) {
  ...
  try {
    if (newEnd === first) {
      const previous = bindings.find(
        (binding) => binding.path[0] === first - 1 && binding.outerBlock
      )?.outerBlock
      const following = bindings.find(
        (binding) => binding.path[0] === oldEnd && binding.outerBlock
      )?.outerBlock
      if (
        previous !== undefined &&
        previous.followingSource?.length === start.range.start - previous.range.end
      ) {
        return { start: previous.range.end, end: end.range.end, insert: '' }
      }
      if (
        following !== undefined &&
        end.followingSource?.length === following.range.start - end.range.end
      ) {
    ...
    while (availableSource.length < (previousSpelling?.length ?? 0)) {
      ...
      const next = bindings.find(
        (binding) => binding.outerBlock?.range.start === position
      )?.outerBlock
    ...
    const ownedEnd =
      ...
      previousSpelling.length > originalSource.length &&
      availableSource.startsWith(previousSpelling)
        ? start.range.start + previousSpelling.length
        : end.range.end
    const ending =
      ownedEnd === end.range.end ? (end.source.match(/(?:\r\n|\r|\n)+$/u)?.[0] ?? '') : ''
    const insert = generated.replace(/(?:\r\n|\r|\n)+$/u, '') + ending
