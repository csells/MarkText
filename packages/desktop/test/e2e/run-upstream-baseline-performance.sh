...
cd "${REPO_ROOT}/packages/desktop"
MARKTEXT_UPSTREAM_PACKAGED_APP="${MOUNTPOINT}/marktext.app" \
MARKTEXT_UPSTREAM_OUTPUT="${OUTPUT}" \
MARKTEXT_UPSTREAM_RUN_ID="${RUN_ID}" \
MARKTEXT_UPSTREAM_EVIDENCE_CLASS="${EVIDENCE_CLASS}" \
MARKTEXT_UPSTREAM_WARMUP_SAMPLES="${WARMUP_SAMPLES}" \
MARKTEXT_UPSTREAM_MEASURED_SAMPLES="${MEASURED_SAMPLES}" \
...
MARKTEXT_UPSTREAM_PACKAGE_MANAGER="${PACKAGE_MANAGER}" \
MARKTEXT_UPSTREAM_NODE_VERSION="${NODE_VERSION}" \
MARKTEXT_UPSTREAM_PLAYWRIGHT_VERSION="${PLAYWRIGHT_VERSION}" \
MARKTEXT_UPSTREAM_LOCKFILE_SHA256="${LOCKFILE_SHA256}" \
MARKTEXT_UPSTREAM_PRODUCER_SHA256="${PRODUCER_SHA256}" \
MARKTEXT_UPSTREAM_PROBE_SHA256="${PROBE_SHA256}" \
MARKTEXT_UPSTREAM_LAUNCHER_SHA256="${LAUNCHER_SHA256}" \
  "${PNPM[@]}" exec playwright test \
  --config test/e2e/playwright.upstream-baseline-performance.config.ts \
  --workers=1 --reporter=line "${PLAYWRIGHT_ARGS[@]}"
