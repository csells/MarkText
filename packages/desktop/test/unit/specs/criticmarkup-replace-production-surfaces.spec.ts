...
describe('CriticMarkup Replace production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Replace command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.replace',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_REPLACE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.replace',
        invoke: () => clickEditMenu('menu.edit.replace')
      }
    ] as const
