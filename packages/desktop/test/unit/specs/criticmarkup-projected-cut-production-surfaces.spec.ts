...
describe('CriticMarkup projected-cut production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Cut command and Edit menu surfaces through the Electron native cut runtime', () => {
    const surfaces = [
      {
        itemId: 'command:edit.cut',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_CUT, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.cut',
        invoke: () => clickEditMenu('menu.edit.cut')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      nativeCut.mockClear()
      invoke()
      expect(nativeCut, itemId).toHaveBeenCalledOnce()
    }
  })
})
