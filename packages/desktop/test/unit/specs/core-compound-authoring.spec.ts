...
it.each(['markup-edits', 'track-edits'] as const)('recovers %s as one acknowledged transaction with exact undo and redo', async kind => {
  ...
  try {
    expect(await lease.binding.submit({ kind, edits, projections: [] }).acknowledged).toMatchObject({ type: 'applied' })
    lease.faultView(new Error('Recover the acknowledged compound operation'))
    const recovered = await manager.recover(lease)
    const expected = kind === 'markup-edits'
      ? '- {++first++}{>>note<<}\n  - second\n  - third\n'
      : '- {++first++}{>>note<<}\n{++  ++}- second\n{++  ++}- third\n'
...
it('rejects overlapping sparse authoring edits with a typed outcome and unchanged source', async() => {
  ...
  try {
    expect(await binding.submit({
      kind: 'markup-edits',
      projections: [],
      edits: [{ start: 0, end: 3, insert: 'a' }, { start: 2, end: 5, insert: 'b' }]
    }).acknowledged).toMatchObject({ type: 'rejected', accepted: false, reason: 'author-invalid' })
