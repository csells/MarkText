...
for (const ending of ['\r', '\r\n', '\n']) {
  it('creates a body line when typing immediately between empty code fences, ' + JSON.stringify(ending), async() => {
    ...
    expect(adapter.accept({ source: 'user', prevDoc: initial.view.state, doc: first, op: [0, 'text', { es: ['x'] }] })).toBe('accepted')
    expect(adapter.accept({ source: 'user', prevDoc: first, doc: second, op: [0, 'text', { es: [1, 'y'] }] })).toBe('accepted')
    await adapter.settled()
    expect(await binding.sourceAtBarrier()).toMatchObject({ source: '~~~ts' + ending + 'xy' + ending + '~~~' + ending })
  ...
  it.each(['$$', '```'])('inserts into the empty body of %s before its retained newline, ' + JSON.stringify(ending), fence => {
  ...
  it(`maps literal ${JSON.stringify(ending)} and retains exact surrounding source while editing its newline`, () => {
    ...
    for (const [offset, expected] of [[1, `a!${ending}b`], [2, `a${ending}!b`]] as const) {
      const adapter = createMuyaPlainTextSourceEditAdapter(view.bindings[0])
      const text = 'a\nb'.slice(0, offset) + '!' + 'a\nb'.slice(offset)
      const result = adapter.accept({
        source: 'user', prevDoc: view.state, doc: [{ ...view.state[0], text }], op: [0, 'text', { es: [offset, '!'] }]
      })
    ...
    const adapter = createMuyaPlainTextSourceEditAdapter(view.bindings[0])
    const result = adapter.accept({
      source: 'user', prevDoc: view.state, doc: [{ ...view.state[0], text: 'ab' }], op: [0, 'text', { es: [1, { d: '\n' }] }]
    })
    if (result.kind !== 'edit') throw new Error('Expected mapped deletion')
    expect(apply(result.edit)).toBe(`\`\`\`${ending}ab${ending}\`\`\`${ending}`)
  })

  it(`preserves queued literal ${JSON.stringify(ending)} edits and exact history through the actor`, async() => {
    const actor = createCoreActor()
    ...
    expect(adapter.accept({
      source: 'user', prevDoc: initial.view.state, doc: joined, op: [0, 'text', { es: [1, { d: '\n' }] }]
    })).toBe('accepted')
    expect(adapter.accept({
      source: 'user', prevDoc: joined, doc: [{ ...joined[0], text: 'a!b' }], op: [0, 'text', { es: [1, '!'] }]
    })).toBe('accepted')
    release?.()
    await adapter.settled()
    expect(await binding.sourceAtBarrier()).toMatchObject({ source: `\`\`\`${ending}a!b${ending}\`\`\`${ending}` })
    await adapter.history('undo', reconcile)
    expect(await binding.sourceAtBarrier()).toMatchObject({ source: `\`\`\`${ending}ab${ending}\`\`\`${ending}` })
    await adapter.history('undo', reconcile)
    expect(await binding.sourceAtBarrier()).toMatchObject({ source })
    adapter.dispose()
  })
