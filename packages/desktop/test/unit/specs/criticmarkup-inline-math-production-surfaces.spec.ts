...
describe('CriticMarkup Inline Math production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Inline Math command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.inline-math',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_INLINE_MATH, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.inlineMath',
        invoke: () => clickFormatMenu('menu.format.inlineMath')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'inline_math' }]
      ])
    }
  })
})
