...
it.each(['ordinary', 'tracked'] as const)('preserves the actual native %s typing group across every authority acknowledgement', async(lane) => {
  ...
  const reconcile = async() => {
    ...
    if (!adapter.hasPendingEdits()) muya.setContent(structuredClone([...next.view.state]), false, { preserveInputGrouping: true })
  ...
  muya.on('json-change', (change: object) => {
    ...
    expect(lane === 'ordinary' ? adapter.accept(native) : adapter.acceptTracked(native, reconcile)).toBe('accepted')
  })
  try {
