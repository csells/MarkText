...
describe('CriticMarkup Paste as Plain Text production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Paste as Plain Text command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.paste-as-plaintext',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_PASTE_AS_PLAINTEXT, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.pasteAsPlainText',
        invoke: () => clickEditMenu('menu.edit.pasteAsPlainText')
      }
    ] as const
