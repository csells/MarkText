...
const loadComponent = (): Vue.Component => {
  const filename = resolve(dirname(fileURLToPath(import.meta.url)), '../../../src/renderer/src/components/editorWithTabs/CoreReviewCommentEditor.vue')
  const { descriptor } = parse(readFileSync(filename, 'utf8'))
  const compiled = compileScript(descriptor, { id: 'core-review-comment-editor', inlineTemplate: true })
...
const mountEditor = () => {
  const props = Vue.reactive({ modelValue: true, targetId: 'comment:one', defaultText: 'Existing', submitting: false, error: '' })
  const submit = vi.fn()
  const cancel = vi.fn()
...
describe('Core Review comment composer', () => {
  it('submits a multiline draft from the actual form and retains it after failure', async() => {
    ...
    editor.host.querySelector('form')?.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))
    expect(editor.submit).toHaveBeenCalledWith({ targetId: 'comment:one', text: 'First line\nSecond line' })
    ...
    expect(editor.host.querySelector('[role="alert"]')?.textContent).toContain('The document changed. Try again.')
  })
  ...
  it('never silently retargets or discards a draft when review selection changes', async() => {
    ...
    editor.host.querySelector<HTMLButtonElement>('[data-testid="critic-review-comment-cancel"]')?.click()
    await Vue.nextTick()
  ...
  it('uses Cmd/Ctrl+Enter to submit, keeps plain Enter multiline, and Escape cancels', async() => {
    ...
    expect(editor.submit).not.toHaveBeenCalled()
    editor.textarea().dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', metaKey: true, bubbles: true, cancelable: true }))
    expect(editor.submit).toHaveBeenCalledWith({ targetId: 'comment:one', text: 'Draft' })
    editor.props.submitting = true
    await Vue.nextTick()
    editor.textarea().dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true }))
    expect(editor.cancel).not.toHaveBeenCalled()
    editor.props.submitting = false
    await Vue.nextTick()
    editor.textarea().dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', ctrlKey: true, bubbles: true, cancelable: true }))
