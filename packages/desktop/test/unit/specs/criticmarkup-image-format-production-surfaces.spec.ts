...
describe('CriticMarkup Image formatting production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Image command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.image',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_IMAGE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.image',
        invoke: () => clickFormatMenu('menu.format.image')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'image' }]
      ])
    }
  })
})
