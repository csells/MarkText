...
describe('CriticMarkup Quote Block production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered Quote Block command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.quote-block',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_QUOTE_BLOCK, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.quoteBlock',
        invoke: () => clickParagraphMenu('menu.paragraph.quoteBlock')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'blockquote' }]
      ])
    }
  })
})
