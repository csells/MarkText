...
it('preserves an edited asset occupying the bitmap hash filename', async() => {
  ...
  const imported = await moveImageToFolder(path.join(root, 'note.md'),
    new File([bytes], 'same.png', { type: 'image/png' }), directory)
...
it('reports bitmap write failures other than a filename collision', async() => {
  ...
  await expect(moveImageToFolder(path.join(root, 'note.md'),
    new File([new Uint8Array([1, 2, 3])], 'same.png'), path.join(root, 'assets')))
