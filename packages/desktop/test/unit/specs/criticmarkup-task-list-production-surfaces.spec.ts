...
describe('CriticMarkup Task List production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered Task List command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.task-list',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_TASK_LIST, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.taskList',
        invoke: () => clickParagraphMenu('menu.paragraph.taskList')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'ul-task' }]
      ])
    }
  })
})
