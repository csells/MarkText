...
describe('CriticMarkup HTML Block production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered HTML Block command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.html-block',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_HTML_BLOCK, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.htmlBlock',
        invoke: () => clickParagraphMenu('menu.paragraph.htmlBlock')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'html' }]
      ])
    }
  })
})
