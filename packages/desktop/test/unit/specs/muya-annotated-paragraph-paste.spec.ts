...
describe('ordinary paragraph paste inside existing annotations', () => {
  it.each([
    ...
    'preserves the owner and exact history for $source',
    async({ source, expected, prefix, position }) => {
      ...
      const change = {
        ...
        op: [
          [0, 'text', { es: [prefix, { d: suffix }, 'One'] }],
          [1, { i: inserted }]
        ]
      }
      expect(sourceEditForMuyaTwoParagraphPaste(view.view.bindings, change)).toEqual({
        start: position,
        end: position,
        insert: 'One\n\nTwo'
      })
