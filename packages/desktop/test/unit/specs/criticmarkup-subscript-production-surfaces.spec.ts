...
describe('CriticMarkup Subscript production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Subscript command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.subscript',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_SUBSCRIPT, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.subscript',
        invoke: () => clickFormatMenu('menu.format.subscript')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'sub' }]
      ])
    }
  })
})
