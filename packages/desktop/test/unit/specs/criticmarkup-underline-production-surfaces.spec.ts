...
describe('CriticMarkup Underline production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Underline command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.underline',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_UNDERLINE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.underline',
        invoke: () => clickFormatMenu('menu.format.underline')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'u' }]
      ])
    }
  })
})
