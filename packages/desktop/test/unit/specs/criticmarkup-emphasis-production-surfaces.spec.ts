...
describe('CriticMarkup Emphasis production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Emphasis command and Format menu Italic surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.emphasis',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_EMPHASIS, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.italic',
        invoke: () => clickFormatMenu('menu.format.italic')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'em' }]
      ])
    }
  })
})
