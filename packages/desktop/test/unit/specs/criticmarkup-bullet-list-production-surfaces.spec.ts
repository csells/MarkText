...
describe('CriticMarkup Bullet List production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered Bullet List command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.bullet-list',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_BULLET_LIST, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.bulletList',
        invoke: () => clickParagraphMenu('menu.paragraph.bulletList')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'ul-bullet' }]
      ])
    }
  })
})
