...
const submitMarkup = (binding: EditorCoreBinding, edits: readonly DocumentSourceEdit[]) =>
  binding.submit({ edits, projections: ['markup'] }).acknowledged

const unexpectedBindingCall = (operation: string): never => {
...
const testBinding = (
  ...
  Object.freeze({
    retainSelection: () => unexpectedBindingCall('retain selection'),
    retainedSelectionAtBarrier: () => unexpectedBindingCall('retained selection read'),
    releaseSelection: () => unexpectedBindingCall('release selection'),
    mode: 'core',
    durableSourceAuthority: 'core',
    open: vi.fn(),
    submit: (input: Parameters<EditorCoreBinding['submit']>[0]) =>
      Object.freeze({
        ...
        acknowledged:
          'kind' in input && input.kind === 'source-input'
            ? apply(input.action.edits)
            : 'edits' in input
              ? apply(input.edits)
              : unexpectedBindingCall('history command')
      }),
    sourceSelectionAtBarrier: () => unexpectedBindingCall('source selection barrier'),
    sourceSyntaxAtBarrier: () => {
      throw new Error('Unexpected source syntax read')
    },
    sourceAtBarrier: () => unexpectedBindingCall('source barrier'),
    plainTextViewAtBarrier: () => unexpectedBindingCall('plain-text view barrier'),
    selectionProjectionAtBarrier: () => unexpectedBindingCall('selection projection barrier'),
    reviewItemAtBarrier: () => unexpectedBindingCall('Review item barrier'),
    observe: () => () => {},
    dispose: vi.fn()
  })
...
class ActorBackedCoreWorker implements CoreWorkerLike {
  ...
  private readonly listeners = new Set<(event: MessageEvent<CoreWorkerResponseEnvelope>) => void>()
  ...
  failNextModelSource: Error | undefined
  ...
  get modelOwner(): CoreModelOwner {
    return {
      request: (request) => {
        if (this.modelFailure !== undefined) throw this.modelFailure
        if (request.type === 'source-at-barrier' && this.failNextModelSource !== undefined) {
          this.modelFailure = this.failNextModelSource
          throw this.modelFailure
        }
        this.requests.push(structuredClone({ type: 'core-request', request }))
        return this.actor.handle(request)
      },
  ...
  addEventListener(
    ...
    listener:
      | ((event: MessageEvent<CoreWorkerResponseEnvelope>) => void)
      | ((event: ErrorEvent) => void)
      | ((event: MessageEvent) => void)
  ): void {
    if (type === 'message') {
      this.listeners.add(listener as (event: MessageEvent<CoreWorkerResponseEnvelope>) => void)
    } else if (type === 'error') {
  ...
  removeEventListener(
    ...
    if (type === 'message') {
      this.listeners.delete(listener as (event: MessageEvent<CoreWorkerResponseEnvelope>) => void)
    } else if (type === 'error') {
  ...
  fail(message: string): void {
    this.modelFailure = new Error(message)
  ...
  addEventListener(
    ...
    if (type === 'message') {
      this.messageListeners.add(
        listener as (event: MessageEvent<CoreWorkerResponseEnvelope>) => void
      )
    }
  }
...
describe('Core actor protocol', () => {
  ...
  it('opens once then applies an exact UTF-16 edit through Worker transport', async() => {
    ...
    const binding = createEditorCoreBinding(worker.modelOwner)
  ...
  it('navigates from a retained Review anchor after the document shrinks', async() => {
    const binding = createEditorCoreBinding(new ActorBackedCoreWorker().modelOwner)
    await binding.open({
      documentId: 'shrinking-review.md',
      source: 'word '.repeat(40) + '\n\n{++new++}\n'
    })
    await binding.submit({ edits: [{ start: 0, end: 200, insert: '' }], projections: [] })
      .acknowledged
    await expect(binding.reviewItemAtBarrier('next', 202)).toMatchObject({
      type: 'review-item',
      revision: 2,
      item: null,
      sourceLength: 12
    })
    await expect(binding.reviewItemAtBarrier('previous', 202)).toMatchObject({
      type: 'review-item',
      revision: 2,
      item: { kind: 'addition', range: { start: 2, end: 11 } }
    })
    await expect(binding.sourceAtBarrier()).toMatchObject({ source: '\n\n{++new++}\n' })
    binding.dispose()
  })
  ...
  it('does not alias a Review item across a same-range revision', async() => {
    ...
    await expect(binding.submit(staleTarget).acknowledged).toMatchObject({
      type: 'rejected',
      reason: 'stale-base',
      revision: 2
    })
  ...
  it.each(['A {==outer {==text==}{>>note<<}==} Z\n', 'A {==outer {>>note<<} tail==} Z\n'])(
    'edits a visible nested Comment with exact actor history: %s',
    (original) => {
      const actor = createCoreActor()
      let sequence = 0
      const session = 821
      expect(
        actor.handle({ type: 'open', session, sequence: ++sequence, source: original })
      ).toMatchObject({ type: 'opened', revision: 1 })
      ...
      if (
        reviewed.type !== 'review-item' ||
        reviewed.item === null ||
        reviewed.commentText !== 'note'
      ) {
  ...
  it('keeps nested Comment subdocuments inside the outer Review editor', () => {
    ...
    expect(actor.handle({ type: 'open', session: 822, sequence: 1, source })).toMatchObject({
      type: 'opened',
      revision: 1
    })
    const reviewed = actor.handle({
  ...
  it('expands a visible reference label to its complete link before authoring', () => {
    ...
    const authored = 'See {==[important][ref]==}.[^n]\n\n[ref]: https://example.com\n[^n]: Note.'
    expect(
      actor.handle({
        type: 'open',
        session: 7781,
  ...
  it('snapshots caller-owned recovery history before synchronous model admission', async() => {
    const actor = createCoreActor()
    const port: CoreModelOwner = Object.freeze({
    ...
    await expect(opening).toMatchObject({ type: 'opened', revision: 1 })
  ...
  it('bounds actor-owned history by the engine entry and retained-unit policy', () => {
    ...
    expect(() =>
      createCoreActor(createDocumentCore, {
  ...
  it('restores mixed undo and redo stacks in actor execution order', () => {
    const actor = createCoreActor()
    const entry = (undoInsert: string, redoInsert: string) =>
      Object.freeze({
  ...
  it('admits consecutive model edits before yielding to another task', () => {
    const binding = createEditorCoreBinding(new ActorBackedCoreWorker().modelOwner)
    binding.open({ documentId: 'one.md', source: 'alpha' })
    const first = submitMarkup(binding, [{ start: 5, end: 5, insert: '!' }])
    const second = submitMarkup(binding, [{ start: 6, end: 6, insert: '?' }])
    expect(first).toMatchObject({ type: 'applied', revision: 2, sourceLength: 6 })
    expect(second).toMatchObject({ type: 'applied', revision: 3, sourceLength: 7 })
    expect(binding.sourceAtBarrier()).toMatchObject({ source: 'alpha!?' })
    binding.dispose()
  })
  ...
  it('submits caller projections with stable identity and observes typed outcomes', async() => {
    ...
    await expect(submission.acknowledged).toMatchObject({
      type: 'applied',
      revision: 2,
      change: {
        projections: [{ name: 'markup' }, { name: 'comment' }]
      }
    })
  ...
  it('notifies the owner once when an idle Worker fails, without requiring another request', async() => {
    ...
    expect(onFailure).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ message: 'idle Worker died' })
    )
    worker.fail('duplicate failure')
    await expect(
      port.request({ type: 'source-at-barrier', session: 91, sequence: 2, baseRevision: 1 })
  ...
  it('terminates the actual port Worker through the opt-in test control', async() => {
    ...
    await expect(pending).rejects.toThrow('Core Worker was terminated by the test harness')
    await expect(
      port.request({
        type: 'source-at-barrier',
        session: 14,
  ...
  it('latches an actor failure envelope as a terminal Worker fault', async() => {
    ...
    expect(worker.terminations).toBe(1)
    expect(onFailure).toHaveBeenCalledExactlyOnceWith(
      expect.objectContaining({ message: 'actor terminated unexpectedly' })
    )
...
describe('Core document session manager', () => {
  ...
  it('preserves final handoff failure before abort and drains other sessions', async() => {
    ...
    const manager = createCoreDocumentSessionManager({
      createBinding(documentId) {
        ...
        return createEditorCoreBinding(worker.modelOwner)
      }
    })
    ...
    lease.onHandoff(() => {
      throw new Error('final view cleanup failed')
    })

    const preserveFailure = vi.fn()
    await expect(
    ...
    expect(preserveFailure).toHaveBeenCalledWith(
      expect.objectContaining({ message: 'final view cleanup failed' }),
      lease
    )
  ...
  it('releases the lease and disposes the binding when abort cleanup throws', async() => {
    const worker = new ActorBackedCoreWorker()
    const manager = createCoreDocumentSessionManager({
      createBinding: () => createEditorCoreBinding(worker.modelOwner)
    })
    ...
    expect(worker.terminations).toBe(1)
    expect(() => lease.binding.submit({ edits: [], projections: [] })).toThrow('lease is released')
  ...
  it('releases a handed-off lease even when terminal view cleanup throws', async() => {
    const manager = createCoreDocumentSessionManager({
      createBinding: () => createEditorCoreBinding(new ActorBackedCoreWorker().modelOwner)
    })
  ...
  it('does not expose source after rejection and can explicitly abort the session', async() => {
    ...
    await expect(rejected.acknowledged).toMatchObject({
      type: 'resource',
      revision: 1,
      sourceLength: 5
    })
    await expect(manager.saveBarrier('abort.md')).rejects.toThrow('requires reconciliation')
  ...
  it('returns exact acknowledged bytes only after the save barrier drains', async() => {
    ...
    expect(await Promise.race([barrier.then(() => 'settled'), Promise.resolve('pending')])).toBe(
      'pending'
    )
    ...
    while (worker.requests.filter((item) => item.request.type === 'source-input').length < 2) {
      await new Promise((resolve) => setTimeout(resolve, 0))
    }
  ...
  it('coalesces concurrent source barriers inside one document session', async() => {
    ...
    expect(
      worker.requests.filter((item) => item.request.type === 'source-at-barrier')
    ).toHaveLength(1)
  ...
  it('invalidates a returned source snapshot before its Promise is consumed when a newer edit is submitted', async() => {
    const worker = new ActorBackedCoreWorker()
    const manager = createCoreDocumentSessionManager({
      createBinding: () => createEditorCoreBinding(worker.modelOwner)
    })
  ...
  it('reopens exact disk bytes in a new generation before accepting later edits', async() => {
    ...
    await expect(oldEdit.acknowledged).toMatchObject({ type: 'applied' })
    let crossedViewBarrier = false
    oldLease.settleView(async() => {
      crossedViewBarrier = true
    })
    ...
    await expect(nextEdit.acknowledged).toMatchObject({ type: 'applied' })
  ...
  it('keeps the acknowledged session usable when a replacement generation cannot open', async() => {
    ...
    const manager = createCoreDocumentSessionManager({
      createBinding() {
        bindingIndex += 1
        if (bindingIndex === 1) {
          return createEditorCoreBinding(oldWorker.modelOwner)
        }
        return Object.freeze({
          ...testBinding(() => {
            throw new Error('unexpected replacement submit')
          }),
          open: () => {
  ...
  it('publishes the old generation before replacement and fences its later actions', async() => {
    const workers: ActorBackedCoreWorker[] = []
    const manager = createCoreDocumentSessionManager({
      createBinding() {
        const worker = new ActorBackedCoreWorker()
    ...
    oldLease.binding.observe((event) => {
      observed.push(event.outcome.type)
    })
    const oldEdit = oldLease.binding.submit({
      edits: [{ start: 0, end: 3, insert: 'OLD' }],
      projections: []
    })
    expect(oldEdit.acknowledged).toMatchObject({ type: 'applied' })
    ...
    expect(() => oldLease.binding.submit({ edits: [], projections: [] })).toThrow('released')
    ...
    await expect(manager.saveBarrier('late.md')).resolves.toEqual(
      expect.objectContaining({ revision: 4, source: 'disk bytes!' })
    )
  ...
  it('preserves an edit admitted during replacement candidate creation', async() => {
    const leaseRef: { current?: CoreDocumentViewLease } = {}
    const manager = createCoreDocumentSessionManager({
      createBinding() {
        const owner = new ActorBackedCoreWorker().modelOwner
        return createEditorCoreBinding({
          request: (request) => {
            if (request.type === 'open' && leaseRef.current !== undefined) {
              expect(
                leaseRef.current.binding.submit({
                  edits: [{ start: 0, end: 3, insert: 'OLD' }],
                  projections: []
                }).acknowledged
              ).toMatchObject({ type: 'applied' })
    ...
    manager.open({ documentId: 'reload-race.md', source: 'old source', lineEnding: '\n' })
    const oldLease = manager.lease('reload-race.md')
    leaseRef.current = oldLease
    await expect(
      manager.replace(oldLease, {
    ...
    ).rejects.toThrow('source changed during candidate open')
  ...
  it('recovers a faulted model from its checkpoint and acknowledged command journal', async() => {
    const workers: ActorBackedCoreWorker[] = []
    const manager = createCoreDocumentSessionManager({
      createBinding() {
        const worker = new ActorBackedCoreWorker()
    ...
    await expect(accepted.acknowledged).toMatchObject({
      type: 'applied',
      revision: 2
    })
    ...
    oldLease.binding.observe((event) => {
      observedAfterFault.push(event.outcome.type)
    })
    ...
    expect(await Promise.race([saving.then(() => 'saved'), Promise.resolve('recovering')])).toBe(
      'recovering'
    )
    expect(recoveryWorker.requests.map((envelope) => envelope.request)).toEqual([
      ...
      expect.objectContaining({ type: 'source-at-barrier', baseRevision: 2 })
    ])
    ...
    await expect(next.acknowledged).toMatchObject({ type: 'applied' })
  ...
  it('restores checkpointed Track Changes history after Worker recovery', async() => {
    ...
    await expect(manager.saveBarrier('recover-checkpointed-track.md')).resolves.toMatchObject({
      source: tracked,
      revision: 2
    })
  ...
  it('rebases a post-checkpoint Review resolution onto its recovery generation', async() => {
    ...
    await expect(manager.saveBarrier('recover-checkpointed-review.md')).resolves.toMatchObject({
      source: checkpointed,
      revision: 2
    })
    await expect(
  ...
  it('replays acknowledged Comment editing with its undo history on recovery', async() => {
    ...
    const replacement = await manager.recover(oldLease)
    await expect(manager.saveBarrier('recover-edit-comment.md')).resolves.toMatchObject({
      source: edited,
      revision: 2
    })
    await expect(
  ...
  it('replays atomic bulk Review resolution with its undo history on recovery', async() => {
    ...
    const replacement = await manager.recover(oldLease)
    await expect(manager.saveBarrier('recover-resolve-all.md')).resolves.toMatchObject({
      source: accepted,
      revision: 2
    })
    await expect(
  ...
  it('preserves actor undo history across a checkpointed Worker recovery', async() => {
    ...
    ).toThrow('crashed after checkpoint')
  ...
  it('checkpoints the accepted recovery journal internally before it exceeds policy', async() => {
    ...
    expect(
      oldWorker.requests.filter((envelope) => envelope.request.type === 'source-at-barrier')
    ).toHaveLength(1)
    ...
    ).toThrow('crashed after internal checkpoint')
    ...
    for (let index = 0; index < DOCUMENT_RESOURCE_POLICY_V1.maximumHistoryEntries; index += 1) {
      await expect(
        replacement.binding.submit({
          kind: 'undo',
          projections: []
    ...
    await expect(manager.saveBarrier('bounded.md')).resolves.toMatchObject({
      source: 'x'.repeat(finalSource.length - DOCUMENT_RESOURCE_POLICY_V1.maximumHistoryEntries)
    })
  ...
  it('fails maintenance closed without clearing its checkpoint or journal', async() => {
    ...
    const oldWorker = workers[0]
    if (oldWorker === undefined) throw new Error('Expected original Core Worker')
    oldWorker.failNextModelSource = new Error('automatic recovery maintenance failed')
    ...
    expect(triggering.acknowledged).toMatchObject({ type: 'applied' })
    expect(() =>
      oldLease.binding.submit({
        edits: [
          {
    ...
    const recoveryWorker = workers[1]
    if (recoveryWorker === undefined) throw new Error('Expected recovery Core Worker')
    const recoveryRequests = recoveryWorker.requests.map((envelope) => envelope.request)
    expect(recoveryRequests[0]).toMatchObject({
      type: 'open',
      source: '',
      recoveryHistory: { undo: [], redo: [] }
    })
    expect(recoveryRequests.filter((request) => request.type === 'apply')).toHaveLength(
      journalLimit
    )
    expect(recoveryRequests.at(-1)).toMatchObject({
      type: 'source-at-barrier',
      baseRevision: journalLimit + 1
    })
    expect(recoveryRequests.at(-2)).toMatchObject({
      type: 'apply',
      edits: [{ start: journalLimit - 1, end: journalLimit - 1, insert: 'x' }]
    })
    expect(
      replacement.binding.submit({
    ...
    await expect(manager.saveBarrier('maintenance-failure.md')).resolves.toMatchObject({
      source: `${'x'.repeat(journalLimit)}!`
    })
  ...
  it('keeps held document sessions isolated across view activation and handoff', async() => {
    ...
    await Promise.resolve()
    expect(await Promise.race([handoff.then(() => 'settled'), Promise.resolve('pending')])).toBe(
      'pending'
    )
    ...
    releaseView()
    await expect(held.acknowledged).toMatchObject({
      type: 'applied',
      revision: 2
    })
    await handoff
  ...
  it('refuses handoff and close after a typed submission rejection', async() => {
    ...
    await expect(rejected.acknowledged).toMatchObject({ type: 'resource' })
    await expect(manager.handoff(lease)).rejects.toThrow('requires reconciliation')
    await expect(manager.close('rejected.md')).rejects.toThrow('live view')
  ...
  it('crosses the view delivery barrier before handing off a lease', async() => {
    ...
    expect(worker.requests[1]).toMatchObject({
      request: {
        type: 'source-input',
        action: { edits: [{ start: 15, end: 19, insert: 'TEXT' }] }
      }
    })
    adapter.dispose()
...
const createAttachedCodeMirror = (
  source: string
): Readonly<{
  ...
  const rangeRect = Object.getOwnPropertyDescriptor(rangePrototype, 'getBoundingClientRect')
  const rangeRects = Object.getOwnPropertyDescriptor(rangePrototype, 'getClientRects')
...
describe('CodeMirror Core adapter', () => {
  ...
  it('includes synchronous Source model work in dispatch-to-ack latency', async() => {
    let now = 10
    const actor = createCoreActor()
    const binding = createEditorCoreBinding({
      request(request) {
        if (request.type === 'source-input') now = 17
        return actor.handle(request)
      },
      dispose: () => actor.dispose()
    })
    binding.open({ documentId: 'source-sync-latency.md', source: 'abc' })
    const doc = new codeMirror.Doc('abc')
    const events: CoreAuthorityPerformanceEvent[] = []
    const adapter = createCodeMirrorCoreAdapter(doc, binding, {
      ...sourceCodeCoreAdapterOptions('abc', '\n'),
      performanceTrace: {
        documentId: 'source-sync-latency.md',
        clock: () => now,
        record: (event) => events.push(event)
      }
    })
    try {
      doc.replaceRange('!', { line: 0, ch: 3 }, { line: 0, ch: 3 }, '+input')
      await adapter.settled()
      expect(events.map(({ phase, at }) => ({ phase, at }))).toEqual([
        { phase: 'dispatch', at: 10 },
        { phase: 'ack', at: 17 },
        { phase: 'reconcile', at: 17 }
      ])
  ...
  it('sends a native change as one exact edit without reading a source snapshot', async() => {
    ...
    expect(worker.requests[1]).toMatchObject({
      request: {
        type: 'source-input',
        action: { edits: [{ start, end: start + 4, insert: 'TEXT' }] }
      }
    })
    expect(worker.requests[1]?.request).not.toHaveProperty('source')
    const afterApply = inspectDocumentCore(core)
  ...
  it('keeps Source authority saveable after a stale Review locator is rejected', async() => {
    ...
    await expect(
      adapter.resolve({ kind: 'addition', range: { start: 2, end: 10 } }, 1, 'accept')
  ...
  it('keeps Source authority saveable after an atomic Review history refusal', async() => {
    const source = '{++abcdef++}\n'
    const worker = new ActorBackedCoreWorker(
      createCoreActor(createDocumentCore, { maximumHistoryInsertUnits: 5 })
    )
    const binding = createEditorCoreBinding(worker.modelOwner)
  ...
  it('keeps a Review locator on the same annotation across earlier queued input', async() => {
    ...
    editor.replaceRange('!', { line: 0, ch: 0 }, { line: 0, ch: 0 }, '+input')
    await expect(
      adapter.resolve({ kind: 'addition', range: { start: 2, end: 9 } }, 1, 'accept')
    ).resolves.toMatchObject({ revision: 3 })

    expect(worker.requests.slice(1)).toMatchObject([
      { request: { type: 'source-input', action: { edits: [{ start: 0, end: 0, insert: '!' }] } } },
      {
        request: {
          type: 'resolve',
  ...
  it('routes immediate undo and redo to actor-owned history without CodeMirror history', async() => {
    ...
    expect(worker.requests.slice(-2).map((item) => item.request.type)).toEqual(['undo', 'redo'])
    adapter.dispose()
    binding.dispose()
  })

  it('maps native history across canonical CRLF and Unicode coordinates', async() => {
    const source = 'head 😀\r\n\r\nordinary text\r\n'
    ...
    await vi.waitFor(() => {
      expect(worker.requests.filter((item) => item.request.type === 'source-input')).toHaveLength(1)
    })
    await undoing
  ...
  it('rebases native input authored while native history painting is pending', async() => {
    const worker = new ActorBackedCoreWorker()
    const binding = createEditorCoreBinding(worker.modelOwner)
    ...
    await vi.waitFor(() => {
      expect(worker.requests.filter((item) => item.request.type === 'source-input')).toHaveLength(2)
    })
    expect(worker.requests.at(-1)?.request).toMatchObject({
      type: 'source-input',
      baseRevision: 3,
      action: { edits: [{ start: 3, end: 3, insert: 'E' }] }
    })
  ...
  it('fences deferred native history painting after adapter disposal', async() => {
    const worker = new ActorBackedCoreWorker()
    const binding = createEditorCoreBinding(worker.modelOwner)
  ...
  it('holds composition updates locally and commits one actor transaction at composition end', async() => {
    ...
    editor.replaceRange('に', editor.posFromIndex(start), editor.posFromIndex(start + 4), '+input')
    editor.replaceRange(
      '日本',
      editor.posFromIndex(start),
    ...
    await new Promise((resolve) => setTimeout(resolve, 0))
    expect(worker.requests.filter((item) => item.request.type === 'source-input')).toEqual([])
  ...
  it('keeps distinct ordinary editor operations in distinct undo entries', async() => {
    ...
    await vi.waitFor(() => {
      expect(worker.requests.filter(({ request }) => request.type === 'source-input')).toHaveLength(
        2
      )
    })
  ...
  it('maps a native LF view edit onto the exact canonical CRLF range', async() => {
    ...
    expect(worker.requests[1]).toMatchObject({
      request: {
        type: 'source-input',
        action: { edits: [{ start: 3, end: 4, insert: 'B' }] }
      }
    })
    expect(acknowledged).toMatchObject({
      type: 'applied',
  ...
  it('preserves canonical CRLF bytes across multiline removal and insertion', async() => {
    ...
    expect(worker.requests[1]).toMatchObject({
      request: {
        type: 'source-input',
        action: { edits: [{ start: 3, end: 7, insert: 'B\r\nD' }] }
      }
    })
    expect(acknowledged).toMatchObject({
      type: 'applied',
  ...
  it('maps mixed endings with an explicit insertion policy across later edits', async() => {
    ...
    expect(worker.requests.slice(1).map(({ request }) => request)).toEqual([
      expect.objectContaining({
        type: 'source-input',
        action: expect.objectContaining({ edits: [{ start: 5, end: 6, insert: 'C\r\nD' }] })
      }),
      expect.objectContaining({
        type: 'source-input',
        action: expect.objectContaining({ edits: [{ start: 10, end: 11, insert: 'Z' }] })
      })
    ])
  ...
  it.each([
    ...
    expect(worker.requests[1]).toMatchObject({
      request: { type: 'source-input', action: { edits: [fixture.edit] } }
    })
    adapter.dispose()
    binding.dispose()
  })
  ...
  it('prevents retained CR and LF fusion across an empty deletion', async() => {
    ...
    expect(worker.requests.slice(1).map(({ request }) => request)).toEqual([
      expect.objectContaining({
        type: 'source-input',
        action: expect.objectContaining({ edits: [{ start: 1, end: 4, insert: '\n\r' }] })
      }),
      expect.objectContaining({
        type: 'source-input',
        action: expect.objectContaining({ edits: [{ start: 3, end: 3, insert: 'Z' }] })
      })
    ])
    adapter.dispose()
    binding.dispose()
  ...
  it('keeps far-tail canonical mapping bounded and independent of CodeMirror indexing', async() => {
    ...
    const edits: unknown[] = []
    const binding = testBinding((submitted) => {
    ...
    expect(edits).toEqual([[{ start: source.length - 4, end: source.length - 3, insert: 'T' }]])
  ...
  it('accounts for page records traversed by both canonical lookup directions', () => {
    ...
    expect(afterOffset.pageRecordsScanned).toBeGreaterThan(before.pageRecordsScanned)
    ...
    expect(afterPosition.pageRecordsScanned).toBeGreaterThan(afterOffset.pageRecordsScanned)
    expect(afterPosition.pageRecordsScanned - afterOffset.pageRecordsScanned).toBeLessThanOrEqual(
      256
    )
  })
  ...
  it('does not advance Core for a later-listener canceled change', async() => {
    ...
    editor.on('beforeChange', (_doc: CodeMirror.Doc, change: CodeMirror.EditorChangeCancellable) =>
      change.cancel()
    )
  ...
  it('uses a later-listener update as the one final exact transaction', async() => {
    ...
    editor.on(
      'beforeChange',
      (_doc: CodeMirror.Doc, change: CodeMirror.EditorChangeCancellable) => {
    ...
    expect(worker.requests[1]).toMatchObject({
      request: {
        type: 'source-input',
        action: { edits: [{ start: 0, end: 2, insert: 'AB' }] }
      }
    })
    adapter.dispose()
    binding.dispose()
  ...
  it('keeps mixed canonical endings equivalent under deterministic edit fuzz', async() => {
    ...
    let revision = 1
    let transaction = 0
    const binding = testBinding((edits) => {
      const edit = edits[0]
      if (edit === undefined) throw new Error('missing fuzz edit')
      canonical = canonical.slice(0, edit.start) + edit.insert + canonical.slice(edit.end)
      revision += 1
      transaction += 1
      return Object.freeze({
        type: 'applied',
    ...
    for (let index = 0; index < 200; index += 1) {
      ...
      editor.replaceRange(insert, { line, ch: startCh }, { line, ch: endCh }, '+input')
      await adapter.settled()
