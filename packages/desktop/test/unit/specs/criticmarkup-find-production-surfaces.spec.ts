...
describe('CriticMarkup Find production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Find command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.find',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_FIND, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.find',
        invoke: () => clickEditMenu('menu.edit.find')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send, itemId).toHaveBeenCalledWith('mt::editor-edit-action', 'find')
    }
  })
})
