...
import * as latestViewRefresh from '@/documentConsumers/latestViewRefresh'

const loadComponent = (): Vue.Component => {
  const filename = resolve(
    dirname(fileURLToPath(import.meta.url)),
    '../../../src/renderer/src/components/editorWithTabs/CoreDocumentProjection.vue'
  )
  const { descriptor } = parse(readFileSync(filename, 'utf8'))
  const compiled = compileScript(descriptor, {
    id: 'core-document-projection',
    inlineTemplate: true
  })
  ...
  new Function('require', 'exports', js)((name: string) => {
    ...
    if (name === '@/documentConsumers/latestViewRefresh') return latestViewRefresh
...
describe('Core read-only projection component', () => {
  it('dispatches a relative reader link through the native link owner with its projection scope', () => {
    const core = createDocumentCore()
    const revision = core.open('[Next](next.md)')
    const host = document.createElement('div')
    document.body.append(host)
    const follow = vi.fn()
    const app = Vue.createApp(loadComponent(), {
      projection: core.project(revision, 'revised'),
      kind: 'revised',
      label: 'Revised'
    })
    app.provide('core-projection-follow-link', follow)
    app.mount(host)
    cleanup.push(() => {
      app.unmount()
      host.remove()
    ...
    expect(follow).toHaveBeenCalledWith(
      'next.md',
      host.querySelector('[data-projection="revised"]')
    )
  })
  ...
  it('updates real rendered Original and Revised views without making document content editable', async() => {
    ...
    const props = Vue.reactive({
      projection: { ast: core.project(revision, 'original').ast },
      kind: 'original',
      label: 'Original'
    })
    ...
    expect(host.querySelector('[data-projection="original"]')?.getAttribute('aria-label')).toBe(
      'Original'
    )
    ...
    expect(host.querySelector('[data-projection="revised"]')?.getAttribute('aria-label')).toBe(
      'Revised'
    )
  })
  ...
  it('renders an isolated multiline Comment safely through the same mounted component', () => {
    ...
    const app = Vue.createApp({
      render: () => Vue.h(component, { projection, kind: 'comment', label: 'Comment' })
    })
    app.mount(host)
    cleanup.push(() => {
      app.unmount()
    ...
    expect(host.querySelector('[data-projection="comment"]')?.getAttribute('contenteditable')).toBe(
      'false'
    )
  })
})
