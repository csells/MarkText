...
describe('CriticMarkup Create Paragraph production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Create Paragraph command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.create-paragraph',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_CREATE_PARAGRAPH, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.createParagraph',
        invoke: () => clickEditMenu('menu.edit.createParagraph')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-edit-action', 'createParagraph']
      ])
    }
  })
})
