...
describe('CriticMarkup Duplicate production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Duplicate command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.duplicate',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_DUPLICATE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.duplicate',
        invoke: () => clickEditMenu('menu.edit.duplicate')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-edit-action', 'duplicate']
      ])
    }
  })
})
