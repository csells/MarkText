...
describe('CriticMarkup Ordered List production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered Ordered List command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.order-list',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_ORDERED_LIST, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.orderedList',
        invoke: () => clickParagraphMenu('menu.paragraph.orderedList')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'ol-order' }]
      ])
    }
  })
})
