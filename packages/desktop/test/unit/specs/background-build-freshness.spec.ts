...
const makeOutputTree = (): string => {
  ...
  for (const [relativePath, contents] of [
    ...
    ['main/documentSessionWorker.js', 'worker-v1'],
...
const makeSourceTree = (): string => {
  ...
  for (const [relativePath, contents] of [
    ...
    ['packages/document-view/src/index.ts', 'export const view = true\n'],
    ['packages/document-view/src/style.css', 'view-style'],
    ['packages/document-core/src/index.ts', 'export const core = true\n'],
    ['packages/document-core/src/profile.md', '# core profile\n'],
    ...
    ['packages/document-view/package.json', '{"name":"@marktext/document-view"}\n'],
    ['packages/document-view/tsconfig.json', '{}\n'],
    ['packages/document-core/package.json', '{"name":"@marktext/document-core"}\n'],
...
describe('deterministic background build freshness', () => {
  ...
  it('fingerprints binary, Markdown, package, TypeScript, and lock build inputs', () => {
    ...
    for (const expectedPath of [
      ...
      '../document-view/src/style.css',
      '../document-core/src/profile.md',
      'tsconfig.json',
      'tsconfig.base.json',
      '../document-view/package.json',
      '../document-view/tsconfig.json',
      '../document-core/package.json',
      '../document-core/tsconfig.json',
      '../document-core/tsconfig.build.json',
      '../../package.json',
      '../../pnpm-workspace.yaml',
      '../../.npmrc',
      '../../pnpm-lock.yaml'
