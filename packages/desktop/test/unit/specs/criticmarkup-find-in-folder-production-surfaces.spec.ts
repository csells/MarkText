...
describe('CriticMarkup Find in Folder production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Find in Folder command and Edit menu surfaces through the renderer edit-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:edit.find-in-folder',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_FIND_IN_FOLDER, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.findInFolder',
        invoke: () => clickEditMenu('menu.edit.findInFolder')
      }
    ] as const
