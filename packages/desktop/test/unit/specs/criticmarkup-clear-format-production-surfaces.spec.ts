...
describe('CriticMarkup Clear Format production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Clear Format command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.clear-format',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_CLEAR_FORMAT, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.clearFormat',
        invoke: () => clickFormatMenu('menu.format.clearFormat')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'clear' }]
      ])
    }
  })
})
