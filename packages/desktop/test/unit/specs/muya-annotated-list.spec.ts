...
it.each([
  ...
  { lane: 'ordinary', command: 'blockquote', prefix: '> ', selector: 'blockquote' },
  { lane: 'tracked', command: 'blockquote', prefix: '> ', selector: 'blockquote' },
...
].flatMap(example => ['plain', 'plain\nsecond'].map(text => ({ ...example, text }))))('keeps $lane native $command conversion editable inside an addition containing $text', async({ lane, command, prefix, selector, text }) => {
  ...
  muya.eventCenter.on('json-change', (change: unknown) => {
    admissions.push(lane === 'tracked' ? adapter.acceptTracked(change, reconcile) : adapter.accept(change))
  })
  try {
