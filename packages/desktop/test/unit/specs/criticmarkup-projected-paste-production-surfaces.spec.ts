...
describe('CriticMarkup projected-paste production surfaces', () => {
  beforeAll(() => {
    loadEditCommands({
  ...
  it('routes registered Paste command and Edit menu surfaces through the Electron native paste runtime', () => {
    const surfaces = [
      {
        itemId: 'command:edit.paste',
        invoke: () => CommandManager.execute(COMMANDS.EDIT_PASTE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.edit.paste',
        invoke: () => clickEditMenu('menu.edit.paste')
      }
    ] as const

    for (const { itemId, invoke } of surfaces) {
      nativePaste.mockClear()
      invoke()
      expect(nativePaste, itemId).toHaveBeenCalledOnce()
    }
  })
})
