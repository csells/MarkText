...
describe('CriticMarkup Strike-through production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Strike-through command and Format menu Strikethrough surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.strike',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_STRIKE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.strikethrough',
        invoke: () => clickFormatMenu('menu.format.strikethrough')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'del' }]
      ])
    }
  })
})
