...
describe('CriticMarkup Code Fence production surfaces', () => {
  beforeAll(() => {
  ...
  it('routes registered Code Fence command and Paragraph menu surface through the renderer paragraph-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:paragraph.code-fence',
        invoke: () => CommandManager.execute(COMMANDS.PARAGRAPH_CODE_FENCE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.paragraph.codeFences',
        invoke: () => clickParagraphMenu('menu.paragraph.codeFences')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-paragraph-action', { type: 'pre' }]
      ])
    }
  })
})
