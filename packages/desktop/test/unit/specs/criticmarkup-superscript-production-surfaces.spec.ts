...
describe('CriticMarkup Superscript production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Superscript command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.superscript',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_SUPERSCRIPT, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.superscript',
        invoke: () => clickFormatMenu('menu.format.superscript')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'sup' }]
      ])
    }
  })
})
