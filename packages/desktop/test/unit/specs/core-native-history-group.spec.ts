...
describe('native authoritative undo groups', () => {
  it.each([
    { lane: 'ordinary', checkpoint: false },
    { lane: 'tracked', checkpoint: false },
    { lane: 'ordinary', checkpoint: true },
    { lane: 'tracked', checkpoint: true }
  ])(
    'preserves a $lane typing group through recovery with mid-group checkpoint=$checkpoint',
    async({ lane, checkpoint }) => {
      ...
      for (const [index, text] of ['A', 'B', 'C'].entries()) {
        ...
        const input: EditorCoreSubmitInput =
          lane === 'ordinary'
            ? {
              edits: [{ start, end: start, insert: text }],
              projections: [],
              nativeHistoryGroup: 'view:1'
            }
            : {
              kind: 'track',
  ...
  it.each(['ordinary', 'tracked'] as const)(
    'undoes one %s native typing group atomically and redoes exact source',
    async(lane) => {
      const actor = createCoreActor()
      ...
      for (const [index, text] of ['A', 'B', 'C'].entries()) {
        ...
        expect(await binding.submit(input as EditorCoreSubmitInput).acknowledged).toMatchObject({
          type: 'applied'
        })
      }
      const expected = lane === 'ordinary' ? 'seedABC\n' : 'seed{++ABC++}\n'
      ...
      expect(await binding.submit({ kind: 'redo', projections: [] }).acknowledged).toMatchObject({
        type: 'applied'
      })
      expect(await binding.sourceAtBarrier()).toMatchObject({ source: expected })
      actor.dispose()
  ...
  it('keeps Source edits, native group changes, and reused tokens after undo independent', async() => {
    ...
    const insert = async(start: number, text: string, nativeHistoryGroup?: string) => {
      expect(
        ...
        } as EditorCoreSubmitInput).acknowledged
      ).toMatchObject({ type: 'applied' })
    }
