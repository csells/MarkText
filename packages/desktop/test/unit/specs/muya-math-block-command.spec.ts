...
it.each(
  ['/math', '/m{++a++}th', '/m{>>inside<<}ath'].flatMap((trigger) =>
    ['\n', '\r\n', '\r'].flatMap((ending) =>
      [false, true].map((tracked) => ({ trigger, ending, tracked }))
    )
  )
)(
  'Quick Insert Math consumes $trigger and governs the immediately following key ($ending Track=$tracked)',
  async({ trigger, ending, tracked }) => {
    ...
    const mathSource = '$$' + ending + ending + '$$'
    const converted =
      (tracked ? '{~~' + trigger + '~>' + mathSource + '~~}' : mathSource) +
      ending +
      ending +
      'outside{>>keep<<}' +
      ending
    ...
    try {
      ...
      if (!block) throw new Error('Expected quick-insert trigger')
      block.setCursor(block.text.length, block.text.length, true)
      app.muya.eventCenter.emit('content-change', { block })
      const math = menu.renderArray.find((item: { label: string }) => item.label === 'math-block')
      if (!math) throw new Error('Expected Math Block menu entry')
      menu.selectItem(math)
      ...
      if (!input) throw new Error('Expected live math body')
      ...
      expect(core.project(reopened, 'original').markdown).toBe(
        tracked
          ? trigger.replace('{++a++}', '').replace('{>>inside<<}', '') +
              ending +
              ending +
              'outside' +
              ending
  ...
  async({ source, converted: ordinary, trackedSource, trigger, tracked }) => {
    ...
    try {
      ...
      if (menu) {
        ...
        if (!math) throw new Error('Expected Math Block menu entry')
        menu.selectItem(math)
      } else app.muya.updateParagraph('mathblock')
      expect(app.binding.sourceAtBarrier()).toMatchObject({ source: converted })
      expect(app.muya.getSelection()).toMatchObject({ anchor: { offset: 0 }, focus: { offset: 0 } })
      const input = app.muya.editor.selection.getSelection()?.anchor.block
      if (!input) throw new Error('Expected live math body')
...
it.each([false, true])(
  'front menu creates an empty math body before the next key (Track=%s)',
  async(tracked) => {
    vi.stubGlobal(
      'ResizeObserver',
      class {
    ...
    try {
      ...
      menu.selectItem(new MouseEvent('click'), { label: 'math-block' })
      expect(app.binding.sourceAtBarrier()).toMatchObject({ source: converted })
      expect(app.muya.getSelection()).toMatchObject({ anchor: { offset: 0 }, focus: { offset: 0 } })
      const input = app.muya.editor.selection.getSelection()?.anchor.block
      if (!input) throw new Error('Expected live math body')
...
it.each(
  ...
  'Format Math restores the actual middle caret on undo ($text Track=$tracked)',
  async({ text, tracked }) => {
    const source = text + '\n\nother\n'
    ...
    try {
      ...
      app.muya.editor.activeContentBlock = block
      app.muya.updateParagraph('mathblock')
      expect(app.binding.sourceAtBarrier()).toMatchObject({ source: converted })
      expect(app.muya.getSelection()).toMatchObject({ anchor: { offset: 0 }, focus: { offset: 0 } })
      await app.adapter.history('undo', app.reconcile)
      expect(app.binding.sourceAtBarrier()).toMatchObject({ source })
...
it('front-menu Math still replaces its target when the caret is elsewhere', async() => {
  vi.stubGlobal(
    'ResizeObserver',
    class {
      observe() {}
  ...
  try {
    ...
    expect(app.muya.getSelection()).toMatchObject({ anchor: { offset: 0 }, focus: { offset: 0 } })
    const input = app.muya.editor.selection.getSelection()?.anchor.block
    if (!input) throw new Error('Expected new math body')
    input.domNode.dispatchEvent(
      new InputEvent('beforeinput', {
        inputType: 'insertText',
        data: 'x',
