...
describe('CriticMarkup Hyperlink production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Hyperlink command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.hyperlink',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_HYPERLINK, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.hyperlink',
        invoke: () => clickFormatMenu('menu.format.hyperlink')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'link' }]
      ])
    }
  })
})
