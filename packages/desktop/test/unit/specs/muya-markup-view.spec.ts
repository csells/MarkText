...
describe('Muya Markup editing view', () => {
  it('presents tracked heading conversion as old paragraph and editable new heading', () => {
    ...
    expect(view.bindings.map(binding => [binding.path, binding.text])).toEqual([
      [[0, 'text'], 'plain'], [[1, 'text'], '# title']
    ])
    expect(mappedMuyaSourceRange(view.bindings[1], { start: 7, end: 7 })).toEqual({ start: 17, end: 17 })
    expect(view.decorations.map(item => [item.path, 'arm' in item.mark && item.mark.arm])).toEqual([
      [[0, 'text'], 'old'], [[1, 'text'], 'new']
    ])
  })
  ...
  it('presents a tracked table paste with native cells and retains the old paragraph', () => {
    ...
    expect(view.decorations.filter(item => 'arm' in item.mark && item.mark.arm === 'old')).toHaveLength(1)
    expect(view.decorations.filter(item => 'arm' in item.mark && item.mark.arm === 'new')).toHaveLength(4)
  })
  ...
  it('retains both suggestion arms and maps continued typing inside an Addition', () => {
    ...
    expect(view.decorations.map(item => [item.mark.kind, 'arm' in item.mark ? item.mark.arm : null,
      view.bindings[0].text.slice(item.range.start, item.range.end)]))
      .toEqual([
        ['addition', null, 'new'], ['deletion', null, 'old'],
        ['substitution', 'old', 'left'], ['substitution', 'new', 'right'],
        ['highlight', null, 'focus']
      ])
    expect(mappedMuyaSourceRange(view.bindings[0], { start: 5, end: 5 }))
      .toEqual({ start: 8, end: 8 })
    expect(mappedMuyaSourceRange(view.bindings[0], { start: 3, end: 5 }))
      .toEqual({ start: 6, end: 8 })
    expect(mappedMuyaSourceRange(view.bindings[0], { start: 1, end: 4 })).toBeUndefined()
    expect(view.comments).toEqual([{ annotationRange: { start: 51, end: 61 }, path: [0, 'text'], offset: 25 }])
  ...
  it('keeps the Core inline meaning when substitution arms would invent emphasis', () => {
    ...
    expect(view.bindings[0].syntax.children).toEqual([
      { kind: 'text', range: { start: 0, end: 9 }, attributes: { semanticText: '*oldnew*' }, children: [] }
    ])
    expect(view.bindings[0].segments[0]).toEqual({
      text: { start: 0, end: 4 }, source: { start: 3, end: 7 }, syntax: { start: 1, end: 5 }
    })
  })
  ...
  it('provides a review anchor and editable empty paragraph for a Comment-only document', () => {
    ...
    expect(view.comments).toEqual([{ annotationRange: { start: 0, end: 10 }, path: [0, 'text'], offset: 0 }])
    expect(mappedMuyaSourceRange(view.bindings[0], { start: 0, end: 0 })).toEqual({ start: 10, end: 10 })
  })

  it('carries typed headings, nested list paragraphs and literal blocks without re-recognition', () => {
    const core = createDocumentCore()
    const revision = core.open('# Heading {++new++}\n\n- {--old--} item\n\n```js\n{++literal++}\n```\n')
    const view = createMuyaMarkupView(core.project(revision, 'markup'), revision.annotations)
  ...
  it('retains annotations in quotes, task lists and tables alongside math presentation', () => {
    ...
    expect(view.state).toEqual([
      { name: 'block-quote', children: [{ name: 'paragraph', text: 'quote' }] },
      {
        ...
        meta: { marker: '-', loose: false },
        children: [
          { name: 'task-list-item', meta: { checked: true }, children: [{ name: 'paragraph', text: 'done' }] }
        ]
      },
      {
        name: 'table',
        children: [
          { name: 'table.row', children: [{ name: 'table.cell', text: 'A', meta: { align: 'none' } }] },
          { name: 'table.row', children: [{ name: 'table.cell', text: 'old', meta: { align: 'none' } }] }
        ]
      },
      { name: 'math-block', text: 'x^2', meta: { mathStyle: '' } }
    ])
    expect(view.decorations.map(item => item.mark.kind)).toEqual(['addition', 'highlight', 'deletion'])
    expect(view.bindings.find(binding => binding.text === 'old')?.path)
      .toEqual([2, 'children', 1, 'children', 0, 'text'])
  })
  ...
  it('keeps empty suggestions reviewable at their exact visible position', () => {
    ...
    expect(view.decorations.map(item => [item.mark.kind, 'arm' in item.mark ? item.mark.arm : null, item.range]))
      .toEqual([
  ...
  it('adapts remaining block presentation from the intrinsic AST', () => {
    const core = createDocumentCore()
    const revision = core.open('---\ntitle: review\n---\n\n<div>raw</div>\n\n---\n\n```mermaid\ngraph TD\nA-->B\n```\n\n[ref]: https://example.com\n\nnote[^a]\n\n[^a]: words', { footnotes: true })
    const view = createMuyaMarkupView(core.project(revision, 'markup'), revision.annotations)

    expect(view.state.map(state => state.name)).toEqual([
      'frontmatter', 'html-block', 'thematic-break', 'diagram', 'paragraph', 'paragraph', 'footnote'
    ])
    expect(view.state[0]).toEqual({ name: 'frontmatter', text: 'title: review', meta: { lang: 'yaml', style: '-' } })
    expect(view.state[3]).toEqual({ name: 'diagram', text: 'graph TD\nA-->B', meta: { type: 'mermaid', lang: 'yaml' } })
    expect(view.state[6]).toEqual({ name: 'footnote', meta: { identifier: 'a' }, children: [{ name: 'paragraph', text: 'words' }] })
  })
  ...
  it('preserves setext heading presentation and literal content without a physical final newline', () => {
    ...
    for (const [source, expected] of [
      ...
      ['\tcode\n\tmore', { name: 'code-block', text: 'code\nmore', meta: { type: 'indented', lang: '' } }],
      ['> ```js\n> x\n> y\n> ```', { name: 'block-quote', children: [{ name: 'code-block', text: 'x\ny', meta: { type: 'fenced', lang: 'js' } }] }]
    ] as const) {
      const revision = core.open(source)
      expect(createMuyaMarkupView(core.project(revision, 'markup'), revision.annotations).state).toEqual([expected])
    }
  })
  ...
  it('keeps an empty block quote editable rather than publishing a childless container', () => {
    ...
    expect(view.state).toEqual([{ name: 'block-quote', children: [{ name: 'paragraph', text: '' }] }])
  ...
  it('retains the trailing paragraph created by Enter without inventing one for a final newline', () => {
    ...
    expect(createMuyaMarkupView(core.project(ordinary, 'markup'), ordinary.annotations).state)
    ...
    expect(view.state).toEqual([{ name: 'paragraph', text: 'seed' }, { name: 'paragraph', text: '' }])
    expect(view.bindings[1].path).toEqual([1, 'text'])
    expect(mappedMuyaSourceRange(view.bindings[1], { start: 0, end: 0 })).toEqual({ start: 6, end: 6 })
    ...
    expect(core.apply(entered, [{ ...edit, insert: 'next' }]).revision.source).toBe('seed\n\nnext\n')
  })
  ...
  it('maps trailing paragraph insertion through CriticMarkup and CRLF without rewriting either', () => {
    ...
    expect(core.apply(revision, [{ ...range, insert: 'next' }]).revision.source)
      .toBe('{++seed++}\r\n\r\nnext\r\n')
  })
  ...
  it('retains end Enter when the original file has no final newline', () => {
    ...
    expect(mappedMuyaSourceRange(view.bindings.at(-1)!, { start: 0, end: 0 }))
      .toEqual({ start: revision.sourceLength, end: revision.sourceLength })
  })
