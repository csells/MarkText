...
describe('CriticMarkup Inline Code production surfaces', () => {
  beforeAll(() => {
    loadFormatCommands({
  ...
  it('routes registered Inline Code command and Format menu surface through the renderer format-action channel', () => {
    const surfaces = [
      {
        itemId: 'command:format.inline-code',
        invoke: () => CommandManager.execute(COMMANDS.FORMAT_INLINE_CODE, fakeWindow)
      },
      {
        itemId: 'menu-entry:menu.format.inlineCode',
        invoke: () => clickFormatMenu('menu.format.inlineCode')
      }
    ] as const
    ...
    for (const { itemId, invoke } of surfaces) {
      ...
      expect(send.mock.calls, itemId).toEqual([
        ['mt::editor-format-action', { type: 'inline_code' }]
      ])
    }
  })
})
