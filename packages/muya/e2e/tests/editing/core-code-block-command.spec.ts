...
for (const trigger of ['/code', '/c{++o++}de', '/co{>>inside<<}de']) {
    for (const tracked of [false, true]) {
        test(`Quick Insert Code owns its trigger and next key (${trigger}, Track=${tracked})`, async ({ page }) => {
            const tail = '\n\noutside{>>keep<<}\n';
            ...
            try {
                ...
                await page.locator(quickInsertItem('code-block')).click();
...
for (const text of ['seed', 'se{++e++}d{>>keep<<}']) {
    for (const tracked of [false, true]) {
        test(`Format Code undo restores its actual middle caret (${text}, Track=${tracked})`, async ({ page }) => {
            const source = text + '\n\nother\n';
