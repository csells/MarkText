...
Baseline captured at PR-6a (2026-05-20); refreshed 2026-07-15 with the
native CriticMarkup parser contract (vendored `packages/marked` fork).
...
## Native parser contract deltas (2026-07-15)

The vendored Marked fork (`packages/marked`, `v18.0.5-marktext.1`) adds
source-provenance tracking, token-graph authority, and CriticMarkup boundary
attachments. Every conformance delta it caused is an improvement; no
previously passing example regressed (the ratchet forbids it).

| Suite | Examples now passing | Cause |
|---|---|---|
| CommonMark 0.31 | 84, 89 (Setext headings) | The fork's setext-heading tokenizer rework (exact line envelopes for source provenance, including indented-content and trailing-whitespace lines) now parses these spec forms correctly (84: leading-space heading/underline tolerance; 89: trailing spaces before the underline). |
| GFM 0.29-gfm | 54, 59 (Setext headings) | The same setext forms under the GFM profile. |
