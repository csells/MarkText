  ...
  parse(tokens: Token[]): ParserOutput {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (this.options.extensions?.renderers?.[anyToken.type]) {
        ...
        if (ret !== false || !['parser_residue', 'space', 'hr', 'heading', 'code', 'table', 'blockquote', 'list', 'html', 'def', 'paragraph', 'text'].includes(genericToken.type)) {
          out += ret || '';
          continue;
        }
      }
      ...
      switch (token.type) {
        case 'parser_residue': {
          out += this.renderer.parserResidue(token);
          break;
        }
        case 'space': {
