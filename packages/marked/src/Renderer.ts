...
export class _Renderer<ParserOutput = string, RendererOutput = string> {
  ...
  space(_token: Tokens.Space): RendererOutput {
    return '' as RendererOutput;
  }

  parserResidue(token: Tokens.ParserResidue): RendererOutput {
    const { diagnostic } = token;
    return '<pre class="marked-block-nesting-limit"'
      + ` data-markdown-diagnostic="${diagnostic.code}"`
      + ` data-markdown-depth="${diagnostic.depth}"`
      + ` data-markdown-depth-limit="${diagnostic.limit}"`
      + ' role="note"'
      + ` title="${escapeHtmlEntities(diagnostic.message)}">`
  ...
  def(_token: Tokens.Def): RendererOutput {
    return '' as RendererOutput;
  }
  ...
  hr(_token: Tokens.Hr): RendererOutput {
    return '<hr>\n' as RendererOutput;
  }
  ...
  br(_token: Tokens.Br): RendererOutput {
